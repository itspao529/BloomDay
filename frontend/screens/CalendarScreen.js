...
export default function CalendarScreen() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      setEvents(data.map(e => ({ ...e, date: e.fecha ? e.fecha.split("T")[0] : e.date })));
  ...
  const addEvent = async () => {
    ...
    try {
      setLoading(true);
      await createEvent(eventText, "", selectedDate + "T08:00:00", "");
      setEventText("");
      await fetchEvents();
  ...
  const deleteEventHandler = (id) => {
    Alert.alert("¿Eliminar?", "¿Borrar este evento?", [
      ...
      { text: "Sí", style: "destructive", onPress: async () => {
        try { await deleteEvent(id); await fetchEvents(); } catch (e) {}
      }}
    ]);
  };
  ...
  const openEditModal = (item) => {
    ...
    setEditedTitle(item.titulo || item.title);
    setModalVisible(true);
  };

  const saveEdit = async () => {
    try {
      await updateEvent(editingEvent.id, editedTitle, editingEvent.descripcion || "", editingEvent.fecha || editingEvent.date + "T08:00:00", editingEvent.lugar || "");
  ...
  if (selectedDate) markedDates[selectedDate] = { ...(markedDates[selectedDate] || {}), selected: true, selectedColor: esAdmin ? "#FF6B9D" : "#6BCB77" };
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={[styles.header, { backgroundColor: esAdmin ? "#FFD93D" : "#6BCB77" }]}>
        <View style={[styles.circle, styles.c1]} />
        <Text style={[styles.titulo, { color: esAdmin ? "#2D2D2D" : "#fff" }]}>📅 Calendario</Text>
        <Text style={[styles.subtitulo, { color: esAdmin ? "#2D2D2D99" : "#ffffff99" }]}>del Salón Kinder</Text>
      </View>
      ...
      <View style={styles.calendarContainer}>
        <Calendar
          ...
          theme={{
            ...
            selectedDayBackgroundColor: esAdmin ? "#FF6B9D" : "#6BCB77",
            selectedDayTextColor: "#fff",
            todayTextColor: esAdmin ? "#FF6B9D" : "#6BCB77",
            dayTextColor: "#2D2D2D", textDisabledColor: "#DDD",
            monthTextColor: "#2D2D2D",
            arrowColor: esAdmin ? "#FF6B9D" : "#6BCB77",
            dotColor: esAdmin ? "#FF6B9D" : "#6BCB77",
            textMonthFontWeight: "900", textDayFontWeight: "600",
          }}
        />
      </View>

      {/* Solo maestra puede agregar eventos */}
      {esAdmin && (
        <View style={styles.inputContainer}>
          <View style={styles.inputBox}>
            <Text style={styles.inputIcon}>✏️</Text>
            <TextInput
              placeholder="Agregar evento en esta fecha..."
      ...
      <Text style={styles.seccionTitulo}>
        {selectedDate ? `🗓️ Eventos del ${selectedDate}` : "🗓️ Selecciona una fecha"}
      </Text>
      {selectedEvents.length === 0 && selectedDate && (
        <View style={styles.vacioBox}>
          <Text style={styles.vacioEmoji}>🌟</Text>
          <Text style={styles.vacioText}>No hay eventos este día</Text>
        </View>
      )}
      {!selectedDate && (
        <View style={styles.vacioBox}>
          <Text style={styles.vacioEmoji}>👆</Text>
          <Text style={styles.vacioText}>Toca un día para ver sus eventos</Text>
        </View>
      )}
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <View style={styles.eventCard}>
            <View style={[styles.eventIconBox, { backgroundColor: colores[index % colores.length] }]}>
              <Text style={styles.eventIcon}>{emojis[index % emojis.length]}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.eventText}>{item.titulo || item.title}</Text>
              <Text style={styles.eventDate}>{item.date}</Text>
            </View>
            {/* Solo maestra puede editar/eliminar */}
            {esAdmin && (
              <View style={styles.actions}>
                <TouchableOpacity onPress={() => openEditModal(item)}>
                  <Text style={styles.actionIcon}>✏️</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => deleteEventHandler(item.id)}>
                  <Text style={styles.actionIcon}>🗑️</Text>
                </TouchableOpacity>
              </View>
            )}
      ...
      {/* Modal editar solo para maestra */}
      {esAdmin && (
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.modalInputBox}>
                ...
                <TextInput style={styles.input} value={editedTitle} onChangeText={setEditedTitle} placeholder="Nombre del evento" placeholderTextColor="#BBB" />
              </View>
              <TouchableOpacity style={styles.btnGuardar} onPress={saveEdit}>
